...
export class MessageService implements IMessageService {
    ...
    getRequestErrorsMessage(
        ...
        function prepareConstraint(constraint: string): string {
            const KEYWORD_END = 'Constraint';
            if (constraint.endsWith(KEYWORD_END)) {
                constraint = constraint.slice(0, -KEYWORD_END.length);
            }

            return _.camelCase(constraint);
        }
        ...
        for (const requestError of requestErrors) {
            ...
            let errorContext = requestError.contexts;
            ...
            while (children?.length > 0) {
                ...
                if (children[0].children?.length > 0) {
                    ...
                    children = [];
                    errorContext = children[0].contexts;
                }
            }

            const errors: IErrors[] = [];
            for (let constraint of constraints) {
                const context = errorContext?.[constraint] ?? {};
                constraint = prepareConstraint(constraint);

...
export class TagsModule implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    ...
    this.moduleRef.registerRequestByContextId({ hello: 'word' }, contextId);
    ...
    //   this.moduleRef.resolve(TagsService, contextId),
    // ]);
    // console.log(tagsServices[0] == tagsServices[1]); // true

    // const tagsServices = await Promise.all([
    //   this.moduleRef.resolve(TagsService),
    //   this.moduleRef.resolve(TagsService),
    // ]);
    // console.log(tagsServices[0] == tagsServices[1]); // false
  }
}
